...
public class SimpleIconCache extends BaseIconCache {
    ...
    protected boolean isInstantApp(@NonNull ApplicationInfo info) {
        try {
            Method isInstantAppMethod = ApplicationInfo.class.getDeclaredMethod("isInstantApp");
            isInstantAppMethod.setAccessible(true);
            return (boolean) isInstantAppMethod.invoke(info);
        } catch (Exception e) {
            e.printStackTrace();
            return false;
        }
